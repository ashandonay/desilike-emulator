...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        choices=["dr1", "dr2"],
        help="DESI release subdir in the data path (bao: {data_release}/{cosmo_model}/"
             "{quantity}). Omit for analyses without a data_release split (e.g. shapefit).",
    )
    parser.add_argument(
        "--nn-model",
        type=str,
    ...
    root_data_dir = get_default_save_path(analysis=args.analysis, quantity=args.quantity, cosmo_model=args.cosmo_model, data_release=args.data_release)
