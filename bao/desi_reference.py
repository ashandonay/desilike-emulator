...
# DESI DR1 systematic-error inflation table + the post-emulator scaler now live in
# the dependency-free `desi_syst` module (importable without desilike), so the
# bedcosmo runtime can reuse them. Re-exported here so this stays the canonical
# reference module. Regenerate with `python desi_reference.py --regen-syst`.
from desi_syst import DESI_SYST_INFLATION, _SIGMA_KEYS, apply_desi_syst  # noqa: E402,F401
...
# ---------------------------------------------------------------------------
# `apply_desi_syst` (dict/array scaler) is re-exported from `desi_syst` at the top
# of this module. The per-target vector form below stays here because it needs
# `core.emulator_target_names` (desilike).
