...
# Sparse tracers analysed with an isotropic BAO fit (single qiso dilation,
# monopole-only) — matches DESI Adame+24 and the config-space bundle data vectors
# (obs_ells=[0] for these). All other tracers use the anisotropic qpar/qper fit.
_ISO_TRACERS = ("BGS", "QSO")
...
def build_bao_likelihood(
    ...
    if lean:
        # Config-space σ-generator path (config_space.py): only the fiducial
    ...
    # Observable power-spectrum multipoles used in the Fisher forecast. An
    # isotropic (qiso) fit uses the monopole only (DESI's BGS/QSO convention);
    # the anisotropic (qparqper) fit uses the monopole + quadrupole.
    _kmax = kmax_cov if kmax_cov is not None else 0.30
    klim = {0: [kmin_eff, _kmax, 0.005]}
    if apmode != "qiso":
        klim[2] = [kmin_eff, _kmax, 0.005]
    observable = TracerPowerSpectrumMultipolesObservable(
        data=params,
        klim=klim,
        theory=theory,
...
def generate_dataset(
    ...
    # the Fourier workers expect.
    if worker_fn is None:
        raise ValueError(
            "generate_dataset requires worker_fn (e.g. fourier_space._worker_run_fisher, "
            "fourier_space._worker_run_fisher_sigma, or config_space._worker_xi_sigma)."
        )
    if make_task is None:
        def make_task(s):
            return (s, tracer_bin, zrange, z_eff, param_defaults, area, nz_slices_path)
    ...
    if workers > 1:
        ...
        while len(param_rows) < n_samples:
            ...
            tasks = [make_task(s) for s in draws]
            for sample, target_vals, tb_str in pool.imap_unordered(worker_fn, tasks):
                total_attempts += 1
                if sample is None:
                    failed += 1
        ...
        while len(param_rows) < n_samples:
            ...
            for sample_in in draws:
                total_attempts += 1
                sample, target_vals, tb_str = worker_fn(make_task(sample_in))
                if sample is None:
                    failed += 1
                    if tb_str and tb_str != "non-finite target values" and not printed_exception:
                        printed_exception = True
                        print("First Fisher failure (showing traceback once):")
                        print(tb_str)
                        print(f"Failing sample: {sample_in}")
                    continue
